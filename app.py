...
def extract_entities(nlp, titles, summaries):
    """Extract top named entities from a cluster's headlines and summaries."""
    # Process each title/summary as a short doc — faster than one giant string
    texts = titles + [s for s in summaries if s.strip()]
    docs = list(nlp.pipe(texts, disable=["tok2vec", "tagger", "parser",
    ...
    categories = {}
    for (cat, name), count in entity_counts.items():
    ...
    for cat, items in categories.items():
        ...
        result[cat] = [name for name, _ in items[:4]]
...
if st.button("🔍 Scan Now", type="primary", use_container_width=True):
    ...
    status3 = st.status("Extracting named entities...", expanded=False)
    nlp = load_spacy()

    # Only extract entities for top 10 clusters to keep it fast
    for cl in clusters[:10]:
        members = headlines.loc[cl["headline_indices"]]
        cl["entities"] = extract_entities(
            nlp,
            members["title"].tolist(),
            members["summary"].tolist(),
        )

    # Mark remaining clusters as having no entities
    for cl in clusters[10:]:
        cl["entities"] = {}

    status3.update(label="Named entities extracted", state="complete")
    ...
    for i, cl in enumerate(clusters):
        ...
        with st.container():
            ...
            if cl.get("entities"):
                ...
                for cat in ["People", "Places", "Organizations", "Groups", "Events"]:
                    names = cl["entities"].get(cat, [])
                    if names:
                        icon = entity_icons.get(cat, "")
                        entity_parts.append(
                            f"{icon} **{cat}:** {', '.join(names)}"
                        )
                if entity_parts:
                    st.markdown("  \n".join(entity_parts))
